...
/* loaded from: KitsX-1.0.3-all.jar:dev/darkxx/utils/menu/xmenu/ItemBuilderGUI.class */
public class ItemBuilderGUI {
    ...
    public ItemBuilderGUI removeEnchants() {
        return meta(m -> {
            Set<Enchantment> setKeySet = m.getEnchants().keySet();
            Objects.requireNonNull(m);
            setKeySet.forEach(m::removeEnchant);
        });
    }
    ...
    public ItemBuilderGUI armorColor(Color color) {
        return meta(LeatherArmorMeta.class, meta -> {
            meta.setColor(color);
        });
    }
